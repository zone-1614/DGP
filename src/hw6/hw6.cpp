...
std::string to_string(pmp::Point p) {
    return fmt::format("{}, {}, {}", p[0], p[1], p[2]);
}
...
public:
    ...
    void deformation() {
        ...
        const auto origin_pos = mesh_.positions();
        ...
        for (int i = 0; i < 10; i++) { // 迭代次数
            // local 阶段, 求Lt
            for (auto v : mesh_.vertices()) { tp.enqueue([&, v] { // 添加到线程池里
            ...
            tp.waitAll(); // 等待local阶段全部执行完才能执行global阶段

            // global 阶段, 利用Lt去优化能量, 得到新的坐标b, 下一个local阶段又根据新的坐标得到新的Lt
            std::mutex mtx;
            for (auto v : mesh_.vertices()) { tp.enqueue([&, v] {
    ...
    auto compute_eij() -> pmp::HalfedgeProperty<Eigen::Vector3f> {
        auto eij = mesh_.halfedge_property<Eigen::Vector3f>("eij");
        for (auto h : mesh_.halfedges()) {
            ...
            auto e = from_p - to_p;
            Eigen::Vector3f eij_vector3f(e[0], e[1], e[2]);
            eij[h] = eij_vector3f;
        }
        return eij;
    }
    ...
    pmp::Point delta_p{0.0f,0.0f,0.0f};
};
